...
const MemberItem = createVisualComponent({
  ...
  render(props) {
    ...
    const actions = userContext.isOwner(props.ownerId) || userContext.isOwner(props.member.id) ? [
      {
        icon: "uugds-delete",
    ...
    //@@viewOn:render
    return <Grid.Item>
      <ListItem actionList={actions} key={props.member.id} className={Config.Css.css(`padding: 1rem 1rem;`)}>{`${props.member.name}`}</ListItem>
    </Grid.Item>
    //@@viewOff:render
  },
});
